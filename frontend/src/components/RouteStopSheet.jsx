...
import { getTripStops, getCrowdingPrediction } from '../services/api';
...
const CROWDING_MAP = {
  not_crowded: { label: 'Not Crowded', color: 'text-green-600 bg-green-50 border-green-100' },
  normal:      { label: 'Normal',      color: 'text-yellow-600 bg-yellow-50 border-yellow-100' },
  overcrowded: { label: 'Overcrowded', color: 'text-red-600 bg-red-50 border-red-100' },
};
...
export default function RouteStopSheet({ leg, onClose, vehicles = [] }) {
  ...
  const [selectedDeparture, setSelectedDep]   = useState(0);
  const [crowdingLevel,     setCrowdingLevel] = useState(null);
  const [crowdingConf,      setCrowdingConf]  = useState(null);
  ...
  useEffect(() => {
    ...
    getTripStops(leg.trip_id, leg.start_time, leg.route)
      .then(data => {
        ...
        if (fetched.length > 0) setStops(fetched);
        else if (leg?.stop_times?.length) setStops(leg.stop_times);
      })
      .catch(() => {
        if (leg?.stop_times?.length) setStops(leg.stop_times);
        else setError('Could not load stops');
      })
  ...
  useEffect(() => {
    ...
    getCrowdingPrediction(stopId, depTime)
      .then(data => {
        if (data?.level) {
          setCrowdingLevel(data.level);
          setCrowdingConf(data.confidence);
        }
      })
      .catch(() => {});
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [stops, selectedDeparture]);

  if (!leg) return null;

  const activeDeparture = departures[selectedDeparture] ?? departures[0];
  const crowding        = CROWDING_MAP[crowdingLevel] || null;
    ...
    <div className="fixed bottom-0 left-0 right-0 z-[2000] bg-white rounded-t-2xl shadow-2xl flex flex-col h-[68vh]">
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-100 shrink-0">
        ...
        <div className="ml-auto flex items-center gap-2 shrink-0">
          ...
          {crowding && (
            <span className={`text-[10px] font-semibold px-2.5 py-1 rounded-full uppercase tracking-wide border ${crowding.color}`}>
              {crowding.label}
              {crowdingConf && (
                <span className="ml-1 opacity-60">{Math.round(crowdingConf * 100)}%</span>
              )}
            </span>
          )}
        </div>
...
<p className="text-[10px] text-gray-400 leading-tight">
  Crowding data is generated by AI and may be mistaken. Refer to other
  transit information if needed.
</p>
        ...
        <div className="mx-3 mb-3 px-4 py-2.5 bg-blue-50 rounded-xl flex items-center justify-between border border-blue-100">
          ...
          {crowding ? (
            ...
            <span className="text-[11px] text-gray-400 italic">Predicting...</span>
          )}
        </div>
        ...
        <div className="px-3">
          <div className="flex items-center justify-between mb-2 px-1">
            <span className="text-[12px] font-semibold text-gray-500 uppercase tracking-wide">Stops</span>
            {stops.length > 0 && (
              <span className="text-[11px] font-semibold text-blue-500">{stops.length} stops</span>
            )}
          </div>
                  ...
                  <div key={stop.stop_id || stop.global_stop_id || i}>
                    ...
                    <div className="flex items-stretch">
                      <div className="flex flex-col items-center w-10 shrink-0">
                        ...
                        {!isLast && <div className={`w-0.5 flex-1 min-h-[16px] ${isPassed ? 'bg-gray-200' : 'bg-blue-200'}`} />}
